...
#### load datasets ####
load("data/R_data_exp01.RData")
load("data/R_data_exp02.RData")
load("data/R_data_exp03.RData")
...
all_merged_t99_replaced <- all_merged_t99_replace99 %>%
  ...
  mutate(across(ends_with("_ChangeAvailRep"), ~case_when(
    . == 0 ~23,  # 'Alle verfügbar' ~0,
    . == 1 ~23,  # 'Assistiert und Automatisiertes Fahren verfügbar' ~1,
    . == 2 ~23,  # 'Automatisiert verfügbar' ~2,
    . == 3 ~2,   # 'Assistiert verfügbar' ~3,
    . == 5 ~2,   # 'Automatisiert nicht mehr verfügbar' ~5,
    . == 4 ~0,   # 'Keines verfügbar' ~4,
    . == 6 ~0,   # 'Automatisiert und Assistiert nicht mehr verfügbar' ~6,
    . == 7 ~0,   # 'Keines mehr verfügbar' ~7,
    . == 8 ~0,   # 'Nur Manuell verfügbar' ~8,
    ...
    . == 66 ~66, #  assistiert nicht mehr verfügbar
    TRUE ~ as.numeric(.)))) %>%
...
## Description ChangeAvail
    # 23   correct for: TC02, TC08, TC11;  alle LoA/automatisiert verfügbar (0->23: TC02 u TC11 & 2->23: TC08)
    # 2    correct for: TC06;              L3 nicht mehr verfügbar, nur L2 (23->2: TC06) 
    # 0    correct for: TC10, TC12;        nur L0 verfügbar (23->0: TC10 u TC12)
    ...
    # 66   half correct for TC10, TC12;    "assistiert nicht mehr verfügbar" (nur bei TC10 u TC12 als halbkorrekt werten?! <- kommt bei den TC nicht vor)
    # 1000 incorrect for TC02, TC06, TC8, TC10-TC12; correct for all others; keine Änderung erkannt --> als (in)korrekt werten
  ## Description ChangeAvailReason
    # 0    correct; 'Systemgrenze' or 'Sensorfehler' ~0,
    # 1    incorrect; 'Weiß nicht mehr' ~1,
    # 2    incorrect; 'Nichts erkannt' ~2,
    # 99   incorrect; Sonstiges/falsch
    # 77   incorrect; beschreibt HoW
    # 1000 incorrect: erkennt keine Änderung 
...
all_merged_t99_replaced_aut_comp2_avail_comp <- all_merged_t99_replaced_aut_comp2_avail %>%
  ...
  mutate(TC01_AvailImplem_Rep = ifelse(TC01_ChangeAvailRep == TC01_ImplemAvail, 1,           # 1000 für TC01, TC03, TC04, TC05, TC07, TC09
                                       ifelse(TC01_ChangeAvailRep == 77, 1,                  # 77 für ""
                                              ifelse(TC01_ChangeAvailRep == 88, 1, 0)))) %>% # 88 für ""
  ...
  mutate(TC02_AvailImplem_Rep = ifelse(TC02_ChangeAvailRep == TC02_ImplemAvail, 1, 0)) %>%   # 23 für TC02, TC08, TC11
  ...
  mutate(TC06_AvailImplem_Rep = ifelse(TC06_ChangeAvailRep == TC06_ImplemAvail, 1, 0)) %>%   # 2 für TC06
  mutate(TC10_AvailImplem_Rep = ifelse(TC10_ChangeAvailRep == TC10_ImplemAvail, 1,           # 0 für TC10, TC12
                                       ifelse(TC10_ChangeAvailRep == 66, 0.5, 0))) %>%       # 66=0.5 für "", kommt nicht vor
...
#### save data ####
write_excel_csv(data_all, "data/R_data_all.csv")
save(data_all, file = "data/R_data_all.RData")
